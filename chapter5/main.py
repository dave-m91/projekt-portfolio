...
@app.get("/v0/players/", response_model=list[schemas.Player], summary = "Pobierz dane jendego zawodnika z użyciem identyfikatora.",
         description="Jeśli dysponujesz identyfikatorem SWC zawodnika z innego wywołania API, np. v0_get_players, możesz wywołaać to API z id gracza",
          response_description="Dane jednego zawodnika NFL", operation_id="v0_get_players_by_player_id", tags=["zawodnik"])
def read_players(skip: int = Query(0, description="Liczba elementów do pominięcia na początku wywołania API"),
                 limit: int = Query(100, description="Liczba rekorów, które API ma zwrócić."),
                 minimum_last_changed_date: date = Query(None, description="Minimalna data modyfikacji dla rekorów, które zostaną zwrócone."),
                 first_name: str = Query(None, description="Imię zawodnika"),
