...
def get_leagues(db: Session, skip: int = 0, limit: int = 0, 
                min_last_changed_date: date = None, league_name: str = None):
    query = db.query(models.League).options(joinedload(models.League.teams))
    if min_last_changed_date:
        query = query.filter(models.League.last_changed_name >= min_last_changed_date)
    if league_name:
...
def get_teams(db: Session, skip: int = 0, limit: int = 100, min_last_changed_date: date = None,
    ...
    if min_last_changed_date:
        query = query.filter(models.Team.last_changed_name >= min_last_changed_date)
    if team_name:
